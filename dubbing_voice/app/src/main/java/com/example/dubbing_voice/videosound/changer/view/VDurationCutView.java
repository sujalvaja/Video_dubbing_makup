...
import com.example.dubbing_voice.videosound.changer.utils.CutUtils;
import com.example.dubbing_voice.videosound.changer.utils.VideoCropHelper;
import com.example.dubbing_voice.videosound.changer.adapter.VDurationCutAdapter;
...
public class VDurationCutView extends RelativeLayout implements RangeSlider.OnRangeChangeListener {
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mAdapter != null) {
            Log.d("TAG13", "onDetachedFromWindow: 清除所有bitmap");
    ...
    public void onKeyUp(int type, int leftPinIndex, int rightPinIndex) {
        ...
        Log.d("TAG13", "onKeyUp: " + leftTime + "===" + rightTime);
        mTvTip.setText(String.format("left side : %s, Right side : %s ", CutUtils.duration(leftTime), CutUtils.duration(rightTime)));
    }
